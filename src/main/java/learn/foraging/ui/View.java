...
public class View {
    ...
    public LocalDate getForager() {
        displayHeader(MainMenuOption.VIEW_FORAGERS.getMessage());
        return io.readLocalDate("Select a date [MM/dd/yyyy]: ");
    }
...
//        System.out.println("foragersssss"+foragers);
        for (Forager forager : foragers) {
            io.printf("firstName: %s; LastName: %s; state: %s%n",
    ...
    public void displayKgPerItems(List<String> res){
        if (res.size() == 0) {
            io.println("No items found");
        }
    ...
    public void displayValPerCatagory(List<String> res){
        if (res.size() == 0) {
            io.println("No items found");
        }
